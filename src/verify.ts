...
export async function verifyChallenge<T extends NumberType>(
  ...
  if (options?.issuedAtTimeWindowMs) {
    ...
    if (new Date(challenge.issuedAt) < earliestIssuedAt) {
      throw `This sign in was issued too long ago: ${challenge.issuedAt}. Must be issued less than ${
        options.issuedAtTimeWindowMs / 1000
      } seconds ago`;
    }
  }
  ...
  if (!toSkipSignatureVerification) {
    ...
    await verifyChallengeSignature(chainDriver, address, message, signature, publicKey);
  }
  ...
  if (options?.expectedChallengeParams) {
    ...
    if (options.expectedChallengeParams.assetOwnershipRequirements)
      options.expectedChallengeParams.assetOwnershipRequirements = convertAssetConditionGroup(
        options.expectedChallengeParams.assetOwnershipRequirements,
        convertFunction,
        true
      );
    if (challenge.assetOwnershipRequirements)
      challenge.assetOwnershipRequirements = convertAssetConditionGroup(challenge.assetOwnershipRequirements, convertFunction, true);
    ...
    for (const key of Object.keys(options?.expectedChallengeParams ?? {})) {
      ...
      if (compareObjects(expected, actual) !== true) {
        throw `unexpected value for ${key}: ${JSON.stringify(challenge[key as keyof ChallengeParams<NumberType>])} !== ${JSON.stringify(
          options?.expectedChallengeParams[key as keyof ChallengeParams<NumberType>] ?? 'undefined'
        )}`;
      }
    }
  }
  ...
  if (challenge.resources || challenge.assetOwnershipRequirements) {
    if (!toSkipAssetVerification) {
      ...
      const assetLookupData = await verifyAssets(
        chainDriver,
        challenge.address,
        challenge.resources ?? [],
        challenge.assetOwnershipRequirements ?? undefined,
        options?.balancesSnapshot
      );
  ...
  return {
    message: `Successfully granted access via Blockin`,
    success: true,
    verificationData
  };
}
...
function assertAssetType<T extends NumberType>(asset: AssetDetails<T>): void {
  ...
  if (typeof asset.collectionId !== 'string') {
  ...
  if (
    !asset.assetIds.every(
      (id) => typeof id === 'string' || (typeof id === 'object' && BigInt(id.start) <= BigInt(id.end) && BigInt(id.start) >= 0 && BigInt(id.end) >= 0)
    )
  )
    throw new Error('Invalid assetIds type');
  if (asset.ownershipTimes && !Array.isArray(asset.ownershipTimes)) throw new Error('Invalid ownershipTimes type');
  if (
...
function parseAssetConditionGroup<T extends NumberType>(lines: string[], convertFunction: (item: NumberType) => T): AssetConditionGroup<T> {
  ...
  if (isAllCondition) {
    ...
    if (trimmedLine.includes('(must satisfy for a minimum of')) {
      ...
      options = {
        numMatchesForVerification: convertFunction(numMatches)
      };
    }
...
export function parseChallengeAssets<T extends NumberType, U extends NumberType>(
  challengeString: string,
  convertFunction: (item: NumberType) => T
): AssetConditionGroup<T> | undefined {
...
function parseChallengeAssetDetails<T extends NumberType, U extends NumberType>(
  lines: string[],
  convertFunction: (item: U) => T
  ...
  for (const line of lines) {
    ...
    if (trimmedLine.startsWith('Chain:')) {
      ...
      if (currentAsset) {
        ...
        currentAsset.assetIds.push({
          start: convertFunction(range[0] as U),
          end: convertFunction(range[1] as U)
        } as UintRange<T>);
      }
      ...
      if (currentAsset) {
        ...
        const firstAmount = range[0]
          .substring(0, range[0].length - 5)
          .trim()
          .substring(1); //-5 to account for (min) (max)
        const secondAmount = range[1]
          .substring(0, range[1].length - 5)
          .trim()
          .substring(1);
  ...
  if (challenge.resources) {
    for (const resource of challenge.resources) {
      ...
      if (typeof resource !== 'string') {
        throw `Inputted resource in resources (${resource}) is not a string`;
      }
    }
  }
...
export function generateAssetConditionGroupString<T extends NumberType>(
  ...
  if (andItem['$and'] !== undefined) {
    ...
    for (let i = 0; i < andItem['$and'].length; i++) {
      message += generateAssetConditionGroupString(
        andItem['$and'][i],
        depth + 1,
        i + 1,
        depth == 0 ? '' : `${parentBullet ? parentBullet + '.' : ''}${bulletNumber}`
      );
    }
  } else if (orItem['$or'] !== undefined) {
 ...
 */
export function constructChallengeObjectFromString<T extends NumberType>(
  challenge: string,
  convertFunction: (item: NumberType) => T
): ChallengeParams<T> {
 ...
 * @param signature - The result of signing the message, formatted to the chain's specifications
 */
async function verifyChallengeSignature<T extends NumberType>(
  ...
  await chainDriver.verifySignature(address, message, signature, publicKey);
}
 ...
 */
async function verifyAssets<T extends NumberType>(
  chainDriver: IChainDriver<T>,
  address: string,
  resources: string[],
  assets: AssetConditionGroup<T> | undefined,
