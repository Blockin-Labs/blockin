...
export class Creator extends Blockin {
     ...
     */
    public async makeAssetCreateTxn(from: string, assetName: string, unitName: string, total: number, assetURL: string, assetMetadataHash: string | Uint8Array, defaultFrozen = false, clawback = from): Promise<any> {
        const decimals = 0
        return await this.client.createTxn({
            from,
            total,
            decimals,
            assetName,
     ...
     */
    public async makeAssetOptInTxn(
        from: string,
        to: string,
        ...
        return await this.client.makeAssetTransferTxn(
            from,
            to,
            closeRemainderTo,
            revocationTarget,
     ...
     */
    public async makeAssetTransferTxn(senderAddress: string, receiverAddress: string, assetId: number): Promise<any> {
        const amount = 1;
