...
export const SignInWithBitBadgesButton: React.FC<{
  ...
  noDefaultStyles
    ...
    const handleChildWindowMessage = async (event: MessageEvent) => {
      ...
      if (event.origin === FRONTEND_URL) {
        ...
        if (onSignAndBlockinVerify) {
          await onSignAndBlockinVerify(event.data.message, event.data.signature,
    ...
    return <button
      ...
      {children ?? <div className='flex-center flex-wrap'>
        Sign In with <img src="https://bitbadges.io/images/bitbadgeslogotext.png" style={{ height: 20, marginLeft: 5 }} alt="BitBadges" /> {loading && <Spin style={{ marginLeft: 4 }} />}
      </div>}
    </button>
  }
